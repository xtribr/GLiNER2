...
interface ChartLegend { a: string; b: string; }

function LegendRow({ legend }: { legend?: ChartLegend }) {
  if (!legend) return null;
  return (
    <div style={{ display: 'flex', gap: '10px', marginTop: '3px', fontSize: '8px', color: '#334155' }}>
      <span><svg width="9" height="9" style={{ verticalAlign: 'middle', marginRight: '2px' }}><rect width="9" height="9" fill="#2563eb" rx="1" /></svg>{legend.a}</span>
      <span><svg width="9" height="9" style={{ verticalAlign: 'middle', marginRight: '2px' }}><rect width="9" height="9" fill="#16a34a" rx="1" /></svg>{legend.b}</span>
    </div>
  );
}

export function AreaBars({ diagnosis, legend }: { diagnosis: DiagnosisComparisonResult; legend?: ChartLegend }) {
  ...
  return (
    <div>
      ...
      <LegendRow legend={legend} />
    </div>
  );
}

export function EvolutionLine({ history, legend }: { history: ComparisonYearRow[]; legend?: ChartLegend }) {
...
export function AreaRadar({ diagnosis, legend }: { diagnosis: DiagnosisComparisonResult; legend?: ChartLegend }) {
