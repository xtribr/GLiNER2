...
class YearResolverTest(unittest.TestCase):
    def test_resolves_latest_files_by_year(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            (data_dir / "school_skills_2025.csv").write_text("x\n", encoding="utf-8")
    ...
    def test_prefers_granular_school_skills_when_year_ties(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            data_dir = Path(tmp_dir)
            legacy = data_dir / "desempenho_habilidades_2025.csv"
            granular = data_dir / "school_skills_2025.csv"
            legacy.write_text("legacy\n", encoding="utf-8")
            granular.write_text("granular\n", encoding="utf-8")

            self.assertEqual(find_latest_school_skills_file(data_dir), granular)

    def test_file_sha256_is_stable(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            path = Path(tmp_dir) / "source.csv"
            path.write_text("fonte-real\n", encoding="utf-8")
